...
export function ProviderManager() {
  const [providers, setProviders] =
    React.useState<ProviderEntry[]>(initialProviders);
        ...
        baseList.map(async (p) => {
          ...
          try {
            ...
            if (enc) {
              ...
              mergedDef = {
                ...mergedDef,
                headers: { ...(mergedDef.headers ?? {}), ...hdrs },
              };
            }
          } catch {
            // Ignore parse errors
  ...
  const updateKey = (id: string, key: string) => {
    setProviders((prev) =>
      prev.map((e) => (e.def.id === id ? { ...e, key } : e)),
    );
  };
  ...
  const save = async (p: ProviderEntry) => {
    if (p.def.auth.type !== "none" && p.key) {
      ...
      setProviders((prev) =>
        prev.map((e) =>
          e.def.id === p.def.id ? { ...e, status: "unknown" } : e,
        ),
      );
    }
  };
  ...
  const addCustomProvider = async () => {
    ...
    const idRaw =
      newProv.id.trim() ||
      newProv.name.toLowerCase().replace(/[^a-z0-9]+/g, "-");
    ...
    if (builtins.has(idRaw)) {
      alert(
        "Provider ID conflicts with a built-in. Please choose a different ID (e.g., add '-override').",
      );
      return;
    }
    const id = idRaw;
    ...
    const def: ProviderDefinition = {
      ...
      auth: {
        type: newProv.authType,
        keyName: newProv.keyName || undefined,
      } as any,
    ...
    try {
      if (existingRaw) list = JSON.parse(existingRaw);
    } catch {
      // Ignore parse errors
    }
    ...
    if (idx >= 0 && editId !== def.id) {
      const ok = await confirm(
        "Provider ID Exists",
        `A provider with ID '${def.id}' already exists. Overwrite it?`,
        { variant: "destructive", confirmLabel: "Overwrite" },
      );
      if (!ok) return;
    }
    ...
    <div className="space-y-3" aria-label="Provider Manager">
      ...
      {showHelp && (
        <div className="text-xs text-muted-foreground border rounded-md p-2">
          - Enter API keys for providers you use (stored encrypted in your
          browser).
          <br />
          - Validate to ensure the key works; Save to persist the key.
          <br />
          - You can clear a key by saving an empty value or from browser storage
          controls.
          <br />- Add a custom provider compatible with OpenAI chat API below.
        </div>
      )}
      <Tabs defaultValue="built-in" className="w-full">
        <TabsList>
          <TabsTrigger value="built-in">Built-in Providers</TabsTrigger>
        ...
        <TabsContent value="built-in" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {providers
              .filter((p) =>
                ["openrouter", "ollama", "groq", "anthropic", "gpt5"].includes(
                  p.def.id,
                ),
              )
              .map((p) => {
                        ...
                        {isBuiltin ? (
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const rows = Object.keys(hdrs).map((k) => ({
                                key: k,
                                value: hdrs[k]!,
                              }));
                              setHeaders(rows);
                          ...
                          <>
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setNewProv({
                                  ...
                                  baseUrl: p.def.baseUrl,
                                  authType:
                                    (p.def.auth.type as any) ?? "apiKey",
                                  keyName:
                                    p.def.auth.keyName || "Authorization",
                                  models: (p.def.models || [])
                                    .map((m) => m.id)
                                    .join(","),
                                });
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                const ok = await confirm(
                                  "Delete Provider",
                                  `Are you sure you want to delete '${p.def.name}'? This action cannot be undone.`,
                                  {
                                    variant: "destructive",
                                    confirmLabel: "Delete",
                                  },
                                ...
                                setProviders((prev) =>
                                  prev.filter((e) => e.def.id !== p.def.id),
                                );
                                ...
                                try {
                                  const list: ProviderDefinition[] = raw
                                    ? JSON.parse(raw)
                                    : [];
                                ...
                                localStorage.removeItem(
                                  `sec_provider_headers:${p.def.id}`,
                                );
                                clearProviderKey(p.def.id);
                                // If we were editing this one, reset form
                                setEditId((prev) =>
                                  prev === p.def.id ? null : prev,
                                );
                              }}
                            >
                              Delete
                    ...
                    <CardContent className="flex items-end gap-2">
                      {p.def.auth.type !== "none" ? (
                        <Input
                          ...
                          value={p.key}
                          onChange={(e) =>
                            updateKey(p.def.id, e.currentTarget.value)
                          }
                        />
                      ) : (
        ...
        <TabsContent value="custom" className="space-y-4">
                          ...
                          <>
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                clearProviderKey(p.def.id);
                                try {
                                  await clearProviderKey(`${p.def.id}_headers`);
                                } catch {
                                  // Ignore errors
                                }
                              }}
          ...
          <div className="rounded-md border p-3 space-y-2 mt-4">
            ...
            <Form
              initialValues={newProv}
              validate={(values) => {
                const errs: Record<string, string> = {};
                if (!values.id.trim()) errs.id = "Provider ID is required";
                if (!values.name.trim()) errs.name = "Name is required";
                if (!values.baseUrl.trim())
                  errs.baseUrl = "Base URL is required";
                if (!values.models.trim())
                  errs.models = "At least one model is required";
                return errs;
              }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                <label className="text-xs">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewProv((v) => ({
                        ...v,
                        authType: e.currentTarget.value as any,
                      }))
                    }
                  >
                ...
                <div>
                  <Input
                    ...
                    placeholder="Header key (e.g., Authorization or x-api-key)"
                    value={newProv.keyName}
                    onChange={(e) =>
                      setNewProv((v) => ({
                        ...v,
                  ...
                  {headers.map((h, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <Input
                        ...
                        value={h.key}
                        onChange={(e) =>
                          updateHeaderRow(idx, "key", e.currentTarget.value)
                        }
                      />
                      <Input
                        ...
                        value={h.value}
                        onChange={(e) =>
                          updateHeaderRow(idx, "value", e.currentTarget.value)
                        }
                      />
                      <Button
                        variant="ghost"
                        size="sm"
                        aria-label={
                          headerMask[idx] ? "Show value" : "Hide value"
                        }
