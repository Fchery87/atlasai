...
describe("Provider adapters", () => {
  ...
  it("GPT5 placeholder validate ok and stream yields placeholder", async () => {
    ...
    expect(res.ok).toBe(true);
    const chunks: string[] = [];
    for await (const c of GPT5Adapter.stream(GPT5Def, "", {
      model: "gpt-5-code-preview",
      messages: [{ role: "user", content: "hi" }],
    })) {
      if (c.type === "text") chunks.push(c.data);
    }
    // The new implementation returns HTML content for testing purposes
  ...
  it("stream yields chunks", async () => {
    ...
    const chunks: string[] = [];
    for await (const c of OllamaAdapter.stream(OllamaDef, "", {
      model: "llama3.1",
      messages: [{ role: "user", content: "hi" }],
    })) {
      if (c.type === "text") chunks.push(c.data);
