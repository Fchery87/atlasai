...
const main = async () => {
  ...
  console.log(
    '    docker run --rm -v "%cd%:/repo" ghcr.io/google/osv-scanner:latest -r /repo   (Windows)',
  );
  console.log(
    '    docker run --rm -v "$(pwd):/repo" ghcr.io/google/osv-scanner:latest -r /repo  (macOS/Linux)',
  );
  console.log(
    "- Or install a native binary from: https://github.com/google/osv-scanner/releases",
  );
  console.log(
    "- Or run the GitHub Action workflow: Security Audit Report (Actions tab).",
  );
  process.exit(0);
};
